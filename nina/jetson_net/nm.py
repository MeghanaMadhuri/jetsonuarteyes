...
log = logging.getLogger("nina.jetson_net.nm")
...
class SavedNetwork:
    ...
    #: ``ap`` for hotspot profiles, ``infrastructure`` for normal STA, etc.
    wifi_mode: str = "infrastructure"
...
class NMBackend:
    ...
    def list_saved_wifi(self) -> List[SavedNetwork]:
        ...
        for ln in raw.stdout.splitlines():
            ...
            mode_raw = (self._connection_wifi_mode(uuid) or "infrastructure").lower()
            wifi_mode = "ap" if mode_raw == "ap" else "infrastructure"
            out.append(
                SavedNetwork(
                    id=name,
                    uuid=uuid,
