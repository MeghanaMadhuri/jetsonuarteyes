...
class HoverboardAxisSettings:
    ...
    uses ``NINA_HOVER_FWD_POS_*`` (defaults 2023 / 2073 — forward: ID **12** −5 and ID **13** +5
    from the ±20 pair 2028/2068); straight backward uses ``NINA_HOVER_REV_POS_*``
...
def load_settings(repo_root: Path) -> NinaSettings:
    ...
    hoverboard_axis = HoverboardAxisSettings(
        ...
        forward_pos_right=_env_int("NINA_HOVER_FWD_POS_RIGHT", 2073),
