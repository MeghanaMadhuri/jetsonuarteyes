...
class HoverboardAxisSettings:
    ...
    2064 right; legacy ``NINA_HOVER_NEUTRAL_*`` is read if the ``BRAKE`` vars are unset),
    ``NINA_HOVER_TILT_DEG``, ``NINA_HOVER_MOVING_SPEED``,
    ``NINA_HOVER_SIGN_LEFT`` / ``RIGHT`` (+1 or -1). Straight-line forward
    uses ``NINA_HOVER_FWD_POS_*`` (defaults 2056 / 2064); straight backward uses
    ``NINA_HOVER_REV_POS_*`` (defaults 2031 / 2074). In-place pivots pair
...
def load_settings(repo_root: Path) -> NinaSettings:
    ...
    hoverboard_axis = HoverboardAxisSettings(
        ...
        brake_pos_right=(
            _env_int("NINA_HOVER_BRAKE_POS_RIGHT", 2064)
            if "NINA_HOVER_BRAKE_POS_RIGHT" in os.environ
            else _env_int("NINA_HOVER_NEUTRAL_RIGHT", 2064)
        ),
        forward_pos_left=_env_int("NINA_HOVER_FWD_POS_LEFT", 2056),
        forward_pos_right=_env_int("NINA_HOVER_FWD_POS_RIGHT", 2064),
        backward_pos_left=_env_int("NINA_HOVER_REV_POS_LEFT", 2031),
        backward_pos_right=_env_int("NINA_HOVER_REV_POS_RIGHT", 2074),
