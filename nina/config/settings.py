...
class HoverboardAxisSettings:
    ...
    uses ``NINA_HOVER_FWD_POS_*`` (defaults 2028 / 2068 — **±20** ticks from 2048);
    straight backward uses ``NINA_HOVER_REV_POS_*`` (defaults 2068 / 2028). In-place pivots pair
...
def load_settings(repo_root: Path) -> NinaSettings:
    ...
    hoverboard_axis = HoverboardAxisSettings(
        ...
        forward_pos_left=_env_int("NINA_HOVER_FWD_POS_LEFT", 2028),
