...
| `NINA_NAV_TURN_SEC`            | **0.15** (Drive buttons) | Software hold for **Turn left/right** only (`NINA_DRIVE_TURN_90_SEC` overrides). Motion-cal **Timed turn duration** does **not** apply to those buttons. |
...
| `NINA_HOVER_TURN_PIVOT_BLEND_PCT` | **17**        | Direct 1–100 blend to pivot goals if `NINA_DRIVE_TURN_PIVOT_DEG` unset. |
