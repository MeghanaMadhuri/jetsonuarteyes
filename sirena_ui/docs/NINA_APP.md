...
| `NINA_NAV_TURN_SEC`            | **0** (Drive buttons) | Software hold for **Turn left/right** only (`NINA_DRIVE_TURN_90_SEC` overrides). **0** = command pivot then brake immediately (no `sleep`). Motion-cal **Timed turn duration** does **not** apply to those buttons. |
