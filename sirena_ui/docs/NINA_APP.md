...
`nina.controllers.navigation_manager.NavigationManager`, constructed via
`nina.controllers.navigation_factory.build_navigation_manager` from
`NavigationSettings` (Jetson GPIO + hardware PWM on the 40-pin header).

The GUI flow uses `drive_continuous(...)` for D-pad presses (non-blocking,
no auto-stop) and `set_wheels(...)` for live speed updates and
autonomous-pilot commands. CLI tools use timed `turn_left()` /
`turn_right()` for scripted turns.

For **Pi-only** JYQD bench work (pigpio on a Raspberry Pi, no Jetson app),
see `pi_motor_bridge/README.md` — that path does not integrate with this
GUI.
...
| `NINA_NAV_TURN_SEC`            | **0.01**           | Default duration (seconds) for timed **turn_left** / **turn_right** (`NavigationSettings.turn_duration_sec`). Motion calibration UI clamps saved duration to **0.01–1** s. After tap, lean returns to brake without settle delay. |
| `NINA_DRIVE_TURN_90_SEC`      | (uses **NINA_NAV_TURN_SEC**) | Drive screen Turn left/right: overrides duration when set. |
| `NINA_DRIVE_TURN_PIVOT_DEG`   | **30**             | Timed pivot lean angle vs nominal 90° (hoverboard): blend = deg/90 → default **33%** of full FWD/REV pivot goals. |
| `NINA_HOVER_TURN_PIVOT_BLEND_PCT` | **33**        | Direct 1–100 blend to pivot goals if `NINA_DRIVE_TURN_PIVOT_DEG` unset. |
| `NINA_DRIVE_TURN_90_PCT`       | (app **20**)      | Legacy name; timed hoverboard turns use pivot **angle** blend above, not this duty. D-pad L/R still uses pivot duty + `NINA_HOVER_TURN_SLOW_WHEEL_PCT`. |
...
The CLI `nav-test-direction` and `nav-test-pin` commands drive individual
Jetson header pins for wiring checks.
...
| Drive    | Live BLDCs (`NavigationManager` on Jetson GPIO) | Sim fallback when backend isn't reachable     |
